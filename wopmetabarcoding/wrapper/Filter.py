...
class Filter(ToolWrapper):
    ...
    def run(self):
        ...
        for row in conn.execute(stmt_marker_id).fetchall():
            ...
            #
            ############################################
            # Filter: lfn1 (low frequency noise)
            ############################################
            filter_obj.lfn1_per_replicate(lfn_per_replicate_threshold)
            #
            ############################################
            # Filter: lfn2 (low frequency noise)
            ############################################
            lfn_per_replicate_series = False
            if not lfn_per_replicate_series:
                filter_obj.lfn2_per_variant(lfn_per_variant_threshold)
            else:
                filter_obj.lfn2_per_replicate_series(lfn_per_replicate_series_threshold)
            #
            ############################################
            # Filter: lfn (low frequency noise)
            ############################################
            filter_obj.lfn3_read_count(lfn_read_count_threshold)
            # #
            # # Filter 4: lfn4
            # if not lfn_per_replicate_series:
            ...
            #     filter_obj.min_repln(biosample_list, 2)
            # else:
            #     filter_obj.min_replp(biosample_list, 3)
            #
            # filter_obj.execute_filters()
            # #
            # # Filter: PCR
            ...
            # # Filter: replp
            # if replp is False:
            #     filter_obj.min_repln(biosample_list, 1)
            # else:
            #     filter_obj.min_replp(biosample_list, 3)
            # filter_obj.execute_filters()
            # #
