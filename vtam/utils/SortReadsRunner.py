...
class SortReadsRunner(object):
    ...
    def __init__(self, fasta_information_df, fasta_path, tag_fwd_sequence_list, tag_rev_sequence_list, primer_fwd_sequence_list,
                 primer_rev_sequence_list, alignement_parameters, trimmed_fasta_df):

        self.fasta_path = fasta_path
        self.alignement_parameters = alignement_parameters

        self.tag_fwd_sequence_list = tag_fwd_sequence_list
        self.tag_rev_sequence_list = tag_rev_sequence_list
        self.primer_fwd_sequence_list = primer_fwd_sequence_list
        self.primer_rev_sequence_list = primer_rev_sequence_list

        # # This is the only order allowed of variant_read_count_df columns
        # column_list = ['Run', 'Marker', 'Biosample', 'Replicate', 'TagFwd',
        #                     'PrimerFwd', 'TagRev', 'PrimerRev', 'Fasta']
        ...
        #     sys.exit(1)
        #
        # fasta_information_df = fasta_information_df.apply(lambda x: x.str.lower(), axis=0)
        self.fasta_information_df = fasta_information_df
        self.trimmed_fasta_df = trimmed_fasta_df
        #
    ...
    def run(self):
        ...
        import pdb; pdb.set_trace()
        # self.annotate_reads(reads_trimmed_fasta_path=reads_trimmed_fasta_path)
        ...
        for record in SeqIO.parse(reads_trimmed_fasta_path, "fasta"):
            ...
            annotation_df = self.fasta_information_df.loc[(self.fasta_information_df.TagFwd == tag_fwd) & (self.fasta_information_df.TagRev == tag_rev) & (self.fasta_information_df.PrimerFwd == primer_fwd) & (self.fasta_information_df.PrimerRev == primer_rev) & (self.fasta_information_df.Fasta == fasta_file_name)]
            import pdb; pdb.set_trace()
