...
class FilterRenkonenRunner(object):
    ...
    def get_variant_read_count_delete_df(self, renkonen_distance_quantile):
        ...
        for run_marker_biosample_replicate_row_i, run_marker_biosample_replicate_row in self.variant_read_count_df[[
            ...
            delete_replicate = run_marker_biosample_replicate_renkonen_df.above_renkonen_distance_quantile.sum(
            ) > (nb_replicates - 1) / 2
    ...
    def get_renkonen_distance_for_one_replicate_pair(
        ...
        replicate_pair_vertical_df = replicate_pair_vertical_df.merge(
            N_j_k_df, left_on=[
                'run_id', 'marker_id', 'biosample_id', 'replicate'], right_on=[
        ...
        replicate_pair_horizontal_df = replicate_left_df.merge(
            replicate_right_df, on=[
                'run_id', 'marker_id', 'biosample_id', 'variant_id'], how='outer')
    ...
    def get_renkonen_distance_df_for_all_biosample_replicates(self):
        ...
        for pos, row in self.variant_read_count_df[[
                'run_id', 'marker_id', 'biosample_id']].drop_duplicates().iterrows():

            run_id = row.run_id
            marker_id = row.marker_id
            biosample_id = row.biosample_id

            run_marker_biosample_df = self.variant_read_count_df.loc[(self.variant_read_count_df.run_id == run_id) & (
            ...
            replicate_pairs = list(
                itertools.combinations(
                    run_marker_biosample_df.replicate.unique(), 2))

            for replicate_left, replicate_right in replicate_pairs:

                renkonen_distance = self.get_renkonen_distance_for_one_replicate_pair(
                    run_marker_biosample_df, replicate_left=replicate_left, replicate_right=replicate_right)
