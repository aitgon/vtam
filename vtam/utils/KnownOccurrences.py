...
class KnownOccurrences(object):
    ...
    def __init__(self, known_occurrences_df, readinfo_tsv, engine):
        ...
        self.known_occurrences_df.columns = map(
            str.lower, self.known_occurrences_df.columns)
        # Rename columns
        self.known_occurrences_df.rename(
            {
                'marker': 'marker_name',
                'run': 'run_name',
        ...
        # columns: run_id, marker_id, biosample_id, replicate, variant_id,
        # biosample_type, action, variant_sequence
        self.known_occurrences_ids_df = pandas.DataFrame.from_records(
            self.get_ids_of_run_marker_biosample_replicate())
    ...
    def read_tsv_into_df(known_occurrences_tsv_path):
        ...
        known_occurrences_df = pandas.read_csv(
            known_occurrences_tsv_path, sep="\t", header=0)
    ...
    def check_known_occurrences_tsv(cls, known_occurrences_tsv_path):
        ...
        if not os.path.isfile(known_occurrences_tsv_path):
            raise argparse.ArgumentTypeError(
                "The file '{}' does not exist. Please fix it.".format(known_occurrences_tsv_path))
        elif not os.stat(known_occurrences_tsv_path).st_size > 0:
            raise argparse.ArgumentTypeError(
                "The file '{}' is empty!".format(known_occurrences_tsv_path))
        header_lower = {
            'run',
            'marker',
            'biosample',
        ...
        # contains at least the 'header_lower' columns
        if set(known_occurrences_df.columns) >= header_lower:
            return known_occurrences_tsv_path  # return the tsv_path
        else:
            raise argparse.ArgumentTypeError(
                "The format of file '{}' is wrong. Please look at the example in the VTAM documentation.".format(known_occurrences_tsv_path))
    ...
    def get_ids_of_run_marker_biosample_replicate(self):
        ...
        for row in self.known_occurrences_df.itertuples():
            ...
            with self.engine.connect() as conn:
                ...
                # get run_id ###########
                #
                stmt_select_run_id = sqlalchemy.select([RunModel.__table__.c.id]).where(
                    RunModel.__table__.c.name == run_name)
                run_id_row = conn.execute(stmt_select_run_id).first()
                if run_id_row is None:
                    Logger.instance().error(
                        VTAMexception(
                            "Run {} not found in the DB. The program will exit. "
                ...
                # get marker_id ###########
                #
                stmt_select_marker_id = sqlalchemy.select([MarkerModel.__table__.c.id]).where(
                    MarkerModel.__table__.c.name == marker_name)
                marker_id_row = conn.execute(stmt_select_marker_id).first()
                if marker_id_row is None:
                    Logger.instance().error(
                        VTAMexception(
                            "Marker {} not found in the DB. The program will exit. "
                ...
                # get biosample_id ###########
                #
                stmt_select_biosample_id = sqlalchemy.select([BiosampleModel.__table__.c.id]).where(
                    BiosampleModel.__table__.c.name == biosample_name)
                biosample_id_first = conn.execute(
                    stmt_select_biosample_id).first()
                if biosample_id_first is None:
                    Logger.instance().error(
                        VTAMexception(
                            "Biosample {} not found in the DB. The program will exit. "
                ...
                instance_list.append({'run_id': run_id,
                                      'marker_id': marker_id,
    ...
    def __are_known_variants_coherent_with_db(self):
        ...
        variant_control_df = self.known_occurrences_df[[
        ...
        with self.engine.connect() as conn:
            ...
            for row in self.known_occurrences_ids_df.itertuples():
                ...
                if not (
                        user_variant_sequence is None) and not (
                        str(user_variant_sequence) == 'nan'):

                    stmt_select = sqlalchemy.select(
                        [
                            VariantReadCount.__table__.c.variant_id,
                            Variant.__table__.c.sequence]) .where(
                        Variant.__table__.c.sequence == user_variant_sequence) .where(
                        VariantReadCount.__table__.c.variant_id == Variant.__table__.c.id) .where(
                        VariantReadCount.__table__.c.run_id == user_run_id) .where(
                        VariantReadCount.__table__.c.marker_id == user_marker_id) .where(
                            VariantReadCount.__table__.c.biosample_id == user_biosample_id)
                    ...
                    if math.isnan(
                            user_variant_id):  # User has not given variant id, then keep this variant id

                        self.known_occurrences_df.loc[self.known_occurrences_df.variant_sequence ==
                                                      user_variant_sequence, 'variant_id'] = db_variant_id

                        self.known_occurrences_ids_df.loc[self.known_occurrences_ids_df.variant_sequence ==
                                                          user_variant_sequence, 'variant_id'] = db_variant_id
    ...
    def __are_known_variants_coherent_with_fasta_info_file(self):
        ...
        for row in self.known_occurrences_ids_df.itertuples():
            ...
            try:
                assert (sample_information_df.loc[(sample_information_df['biosample_id'] == biosample_id) & (
                    sample_information_df['marker_id'] == marker_id) & (sample_information_df['run_id'] == run_id)]).shape[0] > 0
            except AssertionError:
                Logger.instance().error(
                    VTAMexception(
                        "Error: Verify in the --known_occurrences file that run_id, marker_id and biosample_id"
    ...
    def get_keep_run_marker_biosample_variant_df(self, variant_tolerate=False):
        ...
        """
        # Get portion of tsv_path with either keep or keep+variant_tolerate
        if variant_tolerate:  # get also variant_tolerate variant
            ...
            run_marker_biosample_variant_keep_df = self.known_occurrences_ids_df.loc[(
                self.known_occurrences_df.action == 'keep').values]
    ...
    def get_delete_run_marker_biosample_variant_df(
        ...
        variant_read_count_mock = run_marker_biosample_mock_df.merge(
            variant_read_count_df, on=['run_id', 'marker_id', 'biosample_id'])
        # Get keep and tolerates variants to remove from mock
        run_marker_biosample_variant_keep_df = self.get_keep_run_marker_biosample_variant_df(
            variant_tolerate=True)
        #  Compute variants that are 1) mock and keep/tolerate (both). 2) Mock
        # but not keep/tolerate (left_only)
        variant_delete_mock_df = variant_read_count_mock.merge(
            run_marker_biosample_variant_keep_df, on=[
                'run_id', 'marker_id', 'biosample_id', 'variant_id'], how='left', indicator=True)
        ...
        variant_delete_negative_df = self.known_occurrences_ids_df.loc[(
            self.known_occurrences_df.biosample_type == 'negative').values, ['run_id', 'marker_id', 'biosample_id']]
        # Inner merge of variants and negative biosamples
        variant_delete_negative_df = variant_delete_negative_df.merge(
            variant_read_count_df, on=['run_id', 'marker_id', 'biosample_id'])
        # Throw replicate and read count
        variant_delete_negative_df = variant_delete_negative_df[[
            'run_id', 'marker_id', 'biosample_id', 'variant_id']] .drop_duplicates(inplace=False)
        ...
        #
        ##########################################################
        #  Get run_id, marker_id, biosample_id, variant_id that are explicitely
        # marked as delete
        variant_delete_real_df = self.known_occurrences_ids_df.loc[(
            self.known_occurrences_df.action == 'delete').values]
        # Remove delete variant that are not explicite, ie in negative
        # biosamples
        ...
        #
        ##########################################################
        variant_delete_df = pandas.concat(
            [variant_delete_mock_df, variant_delete_negative_df, variant_delete_real_df])
