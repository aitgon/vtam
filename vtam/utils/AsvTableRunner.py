...
class AsvTableRunner(object):
    ...
    def __init__(self, engine, variant_read_count_df, taxonomy_tsv=None):
        ...
        self.variant_to_chimera_borderline_df.rename(
            {'filter_delete': 'chimera_borderline'}, axis=1, inplace=True)
    ...
    def run(self):
        ...
        asv_df3 = asv_df3.merge(
            self.variant_to_chimera_borderline_df, on=[
        ...
        if not (self.taxonomy_tsv is None):
            ...
            with self.engine.connect() as conn:
                for df_row in self.variant_df.itertuples():
                    ...
                    stmt_ltg_tax_assign = sqlalchemy.select(
                        [
                            ...
                            tax_assign_model_table.c.blast_db]) .where(
                        tax_assign_model_table.c.variant_id == variant_id)

                    try:
                        variant_id, identity, ltg_rank, ltg_tax_id, blast_db = conn.execute(
                            stmt_ltg_tax_assign).first()
                        tax_assign_list.append(
                            {
                                'variant_id': variant_id,
                                'identity': identity,
                                'ltg_rank': ltg_rank,
                                'ltg_tax_id': ltg_tax_id,
                                'blast_db': blast_db})
                    except TypeError:  # no result
                        pass
            ltg_tax_assign_df = pandas.DataFrame.from_records(
                tax_assign_list, index='variant_id')
            #
            ltg_tax_assign_df = ltg_tax_assign_df.reset_index().merge(
                taxonomy_df,
                left_on='ltg_tax_id',
                right_on='tax_id',
                how="left").set_index('variant_id')
            ltg_tax_assign_df.drop(
                ['tax_id', 'parent_tax_id', 'rank', 'old_tax_id'], axis=1, inplace=True)
            ltg_tax_assign_df = ltg_tax_assign_df.rename(
                columns={'name_txt': 'ltg_tax_name'})
            ...
            tax_id_list = asv_df['ltg_tax_id'].unique(
            ).tolist()  # unique list of tax ids
        ...
        asv_df_final = asv_df1.merge(
            asv_df2, on=[
                'variant_id', 'marker_id', 'run_id'])
        asv_df_final = asv_df_final.merge(
            asv_df3, on=['variant_id', 'marker_id', 'run_id'])
        asv_df_final.drop('marker_id', axis=1, inplace=True)
        asv_df_final.drop('run_id', axis=1, inplace=True)
        asv_df_final.rename(
            {'run_name': 'run', 'marker_name': 'marker'}, axis=1, inplace=True)
