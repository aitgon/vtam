...
class TaxAssignRunner(object):
    ...
    def __init__(self, variant_df, taxonomy_tsv, blast_db_dir, ltg_rule_threshold, include_prop, min_number_of_taxa, num_threads):
        ...
        self.taxonomy_db_df = self.f01_taxonomy_tsv_to_df()
...
"""
        ...
        for target_tax_id in blast_output_df.target_tax_id.unique().tolist():
            lineage_list.append(self.f04_1_tax_id_to_taxonomy_lineage(target_tax_id, self.taxonomy_db_df))
    ...
    def f01_taxonomy_tsv_to_df(self):
        """
        ...
        """
        taxonomy_db_df = pandas.read_csv(self.taxonomy_tsv, sep="\t", header=0,
                                         dtype={'tax_id': 'int', 'parent_tax_id': 'int', 'old_tax_id': 'float'})
        return taxonomy_db_df


    def f04_1_tax_id_to_taxonomy_lineage(self, tax_id, return_tax_name=False):
        """
        ...
        while tax_id != 1:
            ...
            tax_id_row = self.taxonomy_db_df.loc[self.taxonomy_db_df.tax_id == tax_id,]
            # row empty, try to use old_tax_id
            if tax_id_row.shape[0] == 0:
                tax_id_row = self.taxonomy_db_df.loc[self.taxonomy_db_df.old_tax_id == tax_id,]
            ...
            for identity in identity_list:  # For each variant, loop each decreasing identity
                ...
                if tax_lineage_by_variant_id_df.shape[0] > 0:  # If some hits at this identity, enter analysis
                    ...
                    if (identity < self.ltg_rule_threshold
                        ...
                        #
                        ltg_tax_id, ltg_rank = f06_select_ltg(tax_lineage_by_variant_id_df, include_prop=self.include_prop)
                        # ltg_tax_id, ltg_rank = None, None
