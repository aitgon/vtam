...
class OptimizeLFNreadCountAndLFNvariantRunner:
    ...
    def __init__(
        ...
        known_occurrences_df = KnownOccurrences.read_tsv_into_df(
            known_occurrences)
        ...
        for vknown_grouped_key in vknown_grouped.groups:
            ...
            known_occurrences = KnownOccurrences(
                known_occurrences_df=known_occurrences_i_df,
                readinfo_tsv=fasta_info_tsv_path,
                engine=engine)
            keep_run_marker_biosample_variant_df = known_occurrences.get_keep_run_marker_biosample_variant_df(
            ...
            for lfn_read_count_threshold_i in list(
                range(
                    lfn_read_count_threshold,
                    lfn_read_count_threshold_test_max + 1,
                    10)):
                ...
                Logger.instance().debug(
                    "lfn_read_count_threshold: {} (Max {}); count_keep: {} (Max {})" .format(
                        lfn_read_count_threshold_i,
                        lfn_read_count_threshold_test_max,
                        count_keep,
                        count_keep_max))
            ...
            Logger.instance().debug("Upper limit of lfn_read_count_threshold: {}".format(
                lfn_read_count_threshold_current_max))
            ...
            for lfn_variant_or_variant_replicate_threshold_i in [
                ...
                Logger.instance().debug(
                    "lfn_variant_threshold or lfn_variant_replicate_threshold: {} (Max {}); count_keep: {} (Max {})" .format(
                        lfn_variant_or_variant_replicate_threshold_i,
            ...
            Logger.instance().debug(
                "Upper limit of lfn_variant_replicate_threshold: {}".format(
                    lfn_variant_or_variant_replicate_threshold_current_max))
                ...
                range(
                    ...
                    int(
                        (lfn_read_count_threshold_current_max -
                         lfn_read_count_threshold) /
                        nb_points))]
            lfn_variant_or_variant_replicate_threshold_list = [i /
                                                               1000 for i in range(int(lfn_variant_or_variant_replicate_threshold *
                                                                                       1000), int(lfn_variant_or_variant_replicate_threshold_current_max *
                                                                                                  1000) +
                                                                                   1, int((lfn_variant_or_variant_replicate_threshold_current_max *
                                                                                           1000 -
            ...
            for lfn_read_count_threshold_i in lfn_read_count_threshold_list:
                # loop over lfn_variant_or_variant_replicate_threshold: 0.001,
                # 0.002, ...
                for lfn_variant_or_variant_replicate_threshold_i in lfn_variant_or_variant_replicate_threshold_list:
                    ...
                    variant_read_count_remained_delete_negative_df = variant_read_count_remained_df.merge(
                        variant_delete_df, on=[
                            'run_id', 'marker_id', 'biosample_id', 'variant_id']).drop_duplicates(
                        inplace=False)
                    ...
                    if count_keep >= count_keep_max:  # Store results if count_keep maximal
                        out_lfn_variant_row_dic = {
                            ...
                            "occurrence_nb_keep": count_keep,
                            "occurrence_nb_delete": count_delete}
            ...
            out_lfn_variant_or_variant_replicate_df = pandas.DataFrame(
                out_lfn_variant_list)
            # List of columns in order
            column_names = [
                'occurrence_nb_keep',
                'occurrence_nb_delete',
                'lfn_read_count_threshold',
                'lfn_variant_or_variant_replicate_threshold']
            # Reorder columns
            out_lfn_variant_or_variant_replicate_df = out_lfn_variant_or_variant_replicate_df[
                column_names]
            # Sort columns
            out_lfn_variant_or_variant_replicate_df.sort_values(
                by=column_names, ascending=[False, True, True, True], inplace=True)
            # Rename columns depending on whether this is optimize_lfn_variant
            # or is_optimize_lfn_variant_replicate
            if not is_optimize_lfn_variant_replicate:  # optimize lfn variant
                out_lfn_variant_or_variant_replicate_df = out_lfn_variant_or_variant_replicate_df .rename(
                    columns={'lfn_variant_or_variant_replicate_threshold': 'lfn_variant_threshold'})
            else:  # optimize lfn variant replicate
                out_lfn_variant_or_variant_replicate_df = out_lfn_variant_or_variant_replicate_df .rename(
                    columns={'lfn_variant_or_variant_replicate_threshold': 'lfn_variant_replicate_threshold'})
            ...
            variant_read_count_delete_df = variant_read_count_df.merge(
                variant_delete_df, on=['run_id', 'marker_id', 'biosample_id', 'variant_id'])
            variant_read_count_df_instance = VariantReadCountDF(
                variant_read_count_df)

            if not is_optimize_lfn_variant_replicate:  # optimize lfn variant
                N_i_df = variant_read_count_df_instance.get_N_i_df()

                lfn_variant_or_variant_replicate_specific_threshold_df = variant_read_count_delete_df.merge(
                ...
                lfn_variant_or_variant_replicate_specific_threshold_df.sort_values(
                    by='lfn_variant_threshold', ascending=False, inplace=True)
                lfn_variant_or_variant_replicate_specific_threshold_df.drop_duplicates(
                    'variant_id', keep='first', inplace=True)
                lfn_variant_or_variant_replicate_specific_threshold_df = (
                    lfn_variant_or_variant_replicate_specific_threshold_df[
                ...
                lfn_variant_or_variant_replicate_specific_threshold_df = variant_read_count_delete_df.merge(
                    N_ik_df, on=['run_id', 'marker_id', 'variant_id', 'replicate'])
                lfn_variant_or_variant_replicate_specific_threshold_df[
                ...
                lfn_variant_or_variant_replicate_specific_threshold_df.sort_values(
                    by='lfn_variant_replicate_threshold', ascending=False, inplace=True)
                lfn_variant_or_variant_replicate_specific_threshold_df.drop_duplicates(
                    ['variant_id', 'replicate'], keep='first', inplace=True)
                lfn_variant_or_variant_replicate_specific_threshold_df = (
                    lfn_variant_or_variant_replicate_specific_threshold_df[
            ...
            lfn_variant_delete_mock_specific_threshold_df = lfn_variant_or_variant_replicate_specific_threshold_df.merge(
                variant_delete_mock_df, on=['run_id', 'marker_id', 'variant_id'])
            ...
            lfn_variant_delete_negative_specific_threshold_df = lfn_variant_or_variant_replicate_specific_threshold_df.merge(
                variant_delete_negative_df, on=['run_id', 'marker_id', 'variant_id'])
            ...
            lfn_variant_delete_real_specific_threshold_df = lfn_variant_or_variant_replicate_specific_threshold_df.merge(
                variant_delete_real_df, on=['run_id', 'marker_id', 'variant_id'])
            ...
            if not is_optimize_lfn_variant_replicate:  # optimize lfn variant
                ...
                lfn_variant_or_variant_replicate_specific_threshold_df.groupby(
                    by=['run_id', 'marker_id', 'variant_id', 'N_ijk_max', 'N_ik',
                        'lfn_variant_replicate_threshold']).agg(
                    {'biosample_type': lambda x: ','.join(sorted(set(x)))}).reset_index()
            ...
            lfn_variant_or_variant_replicate_specific_threshold_df = lfn_variant_or_variant_replicate_specific_threshold_df.merge(
            ...
            if not is_optimize_lfn_variant_replicate:  # optimize lfn variant
                lfn_variant_or_variant_replicate_specific_threshold_df = \
                    lfn_variant_or_variant_replicate_specific_threshold_df[
                        ['run_name', 'marker_name', 'variant_id', 'N_ijk_max', 'N_i', 'lfn_variant_threshold',
                         'biosample_type']]
                column_names = [
                    'run_name',
                    'marker_name',
                ...
                column_names = [
                    ...
                    'biosample_type']
                lfn_variant_or_variant_replicate_specific_threshold_df = \
                    lfn_variant_or_variant_replicate_specific_threshold_df[column_names]
                lfn_variant_or_variant_replicate_specific_threshold_df.sort_values(
                    by=column_names, inplace=True)

            #
            # Print only when N_ijk_max larger than default
            # lfn_read_count_threshold
            lfn_variant_or_variant_replicate_specific_threshold_df \
            ...
            final_out_lfn_variant_or_variant_replicate_specific_threshold_df = pandas.concat(
                [final_out_lfn_variant_or_variant_replicate_specific_threshold_df,
                 lfn_variant_or_variant_replicate_specific_threshold_df], axis=0)
        ...
        final_out_lfn_variant_or_variant_replicate_specific_threshold_df.rename(
            {'marker_name': 'marker', 'run_name': 'run'}, axis=1, inplace=True)
        ...
        with engine.connect() as conn:
            for variant_id in final_out_lfn_variant_or_variant_replicate_specific_threshold_df.variant_id.unique():
                ...
                if not (variant_sequence_row is None):
                    ...
                    final_out_lfn_variant_or_variant_replicate_specific_threshold_df.loc[(
                        final_out_lfn_variant_or_variant_replicate_specific_threshold_df.variant_id == variant_id).values, 'sequence'] = variant_sequence
        ...
        final_out_lfn_variant_or_variant_replicate_df.to_csv(
            output_file_optimize_lfn_tsv,
            header=True,
            sep='\t',
            index=False,
            float_format='%.6f')
        ...
        final_out_lfn_variant_or_variant_replicate_specific_threshold_df.to_csv(
            output_file_lfn_variant_specific_threshold_tsv,
            header=True,
            sep='\t',
            index=False,
            float_format='%.6f')

    def lfn_read_count_and_lfn_variant(
            self,
            variant_read_count_df,
            is_optimize_lfn_variant_replicate,
            variant_keep_df,
        ...
        if not is_optimize_lfn_variant_replicate:  # optimize lfn variant replicate
            ...
            lfn_filter_runner.mark_delete_lfn_per_Ni_or_Nik_or_Njk(
                lfn_denominator='N_i', threshold=lfn_variant_or_variant_replicate_threshold)
        else:  # optimize lfn variant replicate
            # lfn_filter_runner.f3_f5_lfn_delete_variant_replicate(lfn_variant_or_variant_replicate_threshold)
            lfn_filter_runner.mark_delete_lfn_per_Ni_or_Nik_or_Njk(
                lfn_denominator='N_ik', threshold=lfn_variant_or_variant_replicate_threshold)
        ...
        lfn_filter_runner.mark_delete_lfn_per_Ni_or_Nik_or_Njk(
            lfn_denominator='N_jk', threshold=lfn_biosample_replicate_threshold)
        ...
        variant_read_count_remained_df = f9_delete_min_replicate_number(
            variant_read_count_remained_df, min_replicate_number)
        variant_read_count_remained_df = variant_read_count_remained_df.loc[
            (variant_read_count_remained_df.filter_delete == 0)]
        variant_read_count_remained_df.drop(
            'filter_delete', axis=1, inplace=True)
        ...
        variant_read_count_remained_keep_df = variant_read_count_remained_df.merge(
