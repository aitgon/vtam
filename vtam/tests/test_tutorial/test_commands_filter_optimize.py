...
class TestCommands(unittest.TestCase):
    ...
    def test_03_filter_lfn_variant_cutoff_specific(self):
        ...
        cmd = "vtam filter --db db.sqlite --readinfo sorted/readinfo.tsv --readdir sorted --asvtable asvtable_default.tsv " \
              "-v --log vtam.log --cutoff {lfn_variant_cutoff_specific}".format(**self.args)
        subprocess.run(shlex.split(cmd), cwd=self.outdir_path)
    ...
    def test_05_optimize_pcr_error(self):
    ...
    def test_06_optimize_lfn_read_count_variant(self):
    ...
    def test_07_optimize_lfn_read_count_variant_replicate(self):
