...
class TestTutorialSnakemake(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.args['outdir_path'] = cls.outdir_path
    ...
    def test_01_filter(self):
        ...
        command = "snakemake --cores 1 -d {outdir_path} -s {snake_tuto_data} --config MARKER=mfzr " \
                  "PROJECT=asper1 PACKAGE_PATH={package_path} --until all_one_marker".format(**self.args)
        subprocess.run(shlex.split(command), check=True)

        command = "snakemake --directory {outdir_path} --resources db=1 " \
                    "--snakefile {snakefile} --cores 1 " \
                    "--configfile {snakeconfig_mfzr} --until asvtable_taxa".format(**self.args)
        subprocess.run(shlex.split(command), check=True)

        run1_mfzr_asvtable_default_taxa = os.path.join(self.outdir_path, "asper1/run1_mfzr/asvtable_default_taxa.tsv")
        run1_mfzr_asvtable_default_taxa_bak = os.path.join(self.test_path, "test_files_dryad.f40v5_small/run1_mfzr/asvtable_default_taxa.tsv")
    ...
    #     command = "snakemake --directory {outdir_path} --resources db=1 " \
    #                 "--snakefile {snakefile} --cores 1 " \
    #                 "--configfile {snakeconfig_mfzr} --until optimize".format(**self.args)
    #     import pdb; pdb.set_trace()
    #     subprocess.run_name(shlex.split(command), check=True)
    #
    #     run1_mfzr_asvtable_default_taxa = os.path.join(self.outdir_path, "asper1/run1_mfzr/asvtable_default_taxa.tsv")
