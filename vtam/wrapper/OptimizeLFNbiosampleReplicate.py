...
class OptimizeLFNbiosampleReplicate(ToolWrapper):
    ...
    def run(self):
        ...
        for vknown_grouped_key in vknown_grouped.groups:
            ...
            final_optimize_output_df = pandas.concat(
                [final_optimize_output_df, optimize_output_df], axis=0)
        ...
        final_optimize_output_df.rename(
            {
                'marker_name': 'marker',
                'run_name': 'run',
        ...
        with engine.connect() as conn:
            for variant_id in final_optimize_output_df.variant_id.unique():
                ...
                if not (variant_sequence_row is None):
                    ...
                    final_optimize_output_df.loc[(
                        final_optimize_output_df.variant_id == variant_id).values, 'sequence'] = variant_sequence
        ...
        final_optimize_output_df.sort_values(
            by=[
                'run',
                'marker',
                'round_down',
            ...
            ascending=[
                ...
                True],
            inplace=True)
        final_optimize_output_df.to_csv(
